#!/usr/bin/env python3
"""
PS5: pyproject.toml の存在確認 (Python プロジェクト判定)

Input:  ps4/python-repo_ps4.csv  (--input で変更可)
Output: ps5/ps5_filtered.csv     (--output で変更可)

Usage:
    python3 ps5_filter.py
    python3 ps5_filter.py --input ps4/python-repo_ps4.csv
...
DEFAULT_INPUT = os.path.join(_SCRIPT_DIR, "ps4", "python-repo_ps4.csv")
...
def check_pyproject_toml(owner: str, repo: str, token: str) -> bool:
    """pyproject.toml の存在を GitHub API で確認"""
...
def main():
    parser = argparse.ArgumentParser(
        description="PS5: Filter projects that have pyproject.toml (Python projects)"
    )
    ...
    print("=" * 60, flush=True)
    print("PS5: Python project filtering (pyproject.toml check)", flush=True)
    print("=" * 60, flush=True)
    ...
    for i, row in enumerate(all_rows):
        ...
        owner, repo = parts
        has_file = check_pyproject_toml(owner, repo, token)
        ...
        if has_file:
            ...
            if (i + 1) % 100 == 0 or failed_count <= 5:
                print(f"  [{i+1}/{total}] {name} — no pyproject.toml", flush=True)
