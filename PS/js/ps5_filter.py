#!/usr/bin/env python3
"""
PS5: package.json の存在確認 (npm プロジェクト判定)

Input:  ps4/ps4_all.csv  (--input で変更可)
        ※ submit-ps5.sh が ps4/ps4_*.csv を自動マージして ps4/ps4_all.csv を生成する
Output: ps5/ps5_filtered.csv  (--output で変更可)

Usage:
    # デフォルト (ps4/ps4_all.csv を入力)
    python3 ps5_filter.py

    # 入力ファイルを明示
    python3 ps5_filter.py --input ps4/ps4_all.csv

    # 出力先を指定
    python3 ps5_filter.py --output ps5/ps5_filtered.csv
...
DEFAULT_INPUT = os.path.join(_SCRIPT_DIR, "ps4", "ps4_all.csv")
...
def check_package_json(owner: str, repo: str, token: str) -> bool:
    """package.json の存在を GitHub API で確認"""
...
def main():
    parser = argparse.ArgumentParser(
        description="PS5: Filter projects that have package.json (npm projects)"
    )
    parser.add_argument(
        "--input", type=str, default=DEFAULT_INPUT,
        help=f"入力 CSV ファイル (default: {DEFAULT_INPUT})",
    )
    parser.add_argument(
        "--output", type=str, default=DEFAULT_OUTPUT,
        help=f"出力 CSV ファイル (default: {DEFAULT_OUTPUT})",
    )
    parser.add_argument(
    ...
    print("=" * 60, flush=True)
    print("PS5: npm project filtering (package.json check)", flush=True)
    print("=" * 60, flush=True)
    ...
    for i, row in enumerate(all_rows):
        ...
        owner, repo = parts
        has_pkg = check_package_json(owner, repo, token)
        ...
        if has_pkg:
            ...
            if (i + 1) % 100 == 0 or failed_count <= 5:
                print(f"  [{i+1}/{total}] {name} — no package.json", flush=True)
