...
#SBATCH --mem=100G
#SBATCH --gres=gpu:1
#SBATCH --output=/work/rintaro-k/research/RQ3/RQ3-2/rust/batch/output/%x/%j/out.out
#SBATCH --error=/work/rintaro-k/research/RQ3/RQ3-2/rust/batch/output/%x/%j/err.err

# Usage:
#   sbatch submit.sh --repo-list /work/rintaro-k/research/PS/rust/ps6/ps6_filtered.csv --batch-index 0
...
if [[ -z "$REPO_LIST_FILE" ]]; then
    ...
    echo "  Example: sbatch submit.sh --repo-list /work/rintaro-k/research/PS/rust/ps6/ps6_filtered.csv --batch-index 0"
    exit 1
fi
export REPO_LIST_PATH="${REPO_LIST_FILE}"

TIME_LOG="/work/rintaro-k/research/RQ3/RQ3-2/rust/batch/output/${SLURM_JOB_NAME:-rq3_rust}/${SLURM_JOB_ID:-local}/timing.log"
TOTAL_START=$(date +%s)
...
# ── 作業ディレクトリ ──
OUT_DIR="/work/rintaro-k/research/RQ3/RQ3-2/rust/batch/output/${SLURM_JOB_NAME:-rq3_rust}/${SLURM_JOB_ID:-local}"
mkdir -p "$OUT_DIR"
cd /work/rintaro-k/research/RQ3/RQ3-2/rust
