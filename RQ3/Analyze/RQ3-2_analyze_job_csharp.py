#!/usr/bin/env python3
"""
analyze_job_csharp.py - C# ジョブの final_dataset.csv を集約して精度指標を出力する

Usage:
    python3 analyze_job_csharp.py
    python3 analyze_job_csharp.py --per-job   # ジョブ別の内訳も表示
"""
...
# batch_index 0-43 の最新ジョブ (各 batch_index で CSV が存在するもの)
JOB_IDS = [
    4263701,  # idx=0
    4263705,  # idx=1
    4263707,  # idx=2
...
def find_csv_for_job(job_id: int) -> list[Path]:
    csharp_job_dir = CSHARP_BATCH_ROOT / str(job_id)
    js_job_dir     = BATCH_ROOT / str(job_id)

    if csharp_job_dir.is_dir():
        job_dir   = csharp_job_dir
        data_root = CSHARP_DATA_ROOT
    elif js_job_dir.is_dir():
        job_dir   = js_job_dir
        data_root = DATA_ROOT
    else:
        print(f"[WARN] job {job_id}: ディレクトリが見つかりません ({js_job_dir} / {csharp_job_dir})")
        return []
