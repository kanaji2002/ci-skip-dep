...
# --- 入力パス (環境変数 or CLI で上書き可能) ---
# REPO_LIST_PATH は submit.sh が export するか pipeline_main.py の --repo-list で設定
...
# --- フィルタリング設定 ---
NOT_PACKAGES = [
    "web", "version", "description", "src/main", "ci",
    "test", "release", "start", "license", "name", "test-ci"
]
...
def set_output_dir(csv_path: str, batch_index: int = None):
    """CSV ファイル名ステムに基づいて出力ディレクトリを切り替える。

    batch_index あり:
        中間データ → data_dependency_waste_project/{stem}/batch_{index}/
        datasets   → data_dependency_waste_project/{stem}/datasets/  (共有)
    batch_index なし:
        全パス     → data_dependency_waste_project/{stem}/
    """
...
def get_filtered_project_list_path(repo_list_path: str) -> str:
    """入力CSVパスから filtered CSV パスを導出"""
    basename = os.path.splitext(os.path.basename(repo_list_path))[0]
    return os.path.join(PATHS["filtered"], f"{basename}_filtered.csv")
