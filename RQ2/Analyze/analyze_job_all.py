#!/usr/bin/env python3
"""
analyze_job_all.py - 複数ジョブの final_dataset.csv を集約して精度指標を出力する

Usage:
    python3 analyze_job_all.py
    python3 analyze_job_all.py --per-job   # ジョブ別の内訳も表示
"""
...
def find_csv_for_job(job_id: int) -> list[Path]:
    job_dir = BATCH_ROOT / str(job_id)
    if not job_dir.is_dir():
        print(f"[WARN] job {job_id}: ディレクトリが見つかりません ({job_dir})")
        return []
