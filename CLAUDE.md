...
- PS0 : JavaScript  Download in 2026/3/3  　10 star            → 294,477 repo
- PS1 : Over 10 commit (2024/3/1 - 2026/3/1).    → 46,249 repo
- PS2  : Having  `.github/workflows`                   →  22,716 repo
- PS3 : Ober 10 CI data.                                          → 18,286 repo
- PS4 : Is there dependencies ( package managment system)
    - JS : Have a  `package.json`It’s mean it is  npm package .  → 14,705 repos
    - Python : .toml
    - Rust : Cargo.toml
    - C# : .csproj
- PS5 : Size over 10 MB
...
- PS6 : Using test tool
    - JS : Have a npm test
    - Python  : Have a pytest
    - Rust :  Is there tests/ directoly in Cargo.toml
    - C# :  Refer to  xUnit in .csproj
- PS7 : Clone and execute test, then calculate coverage
    - line coverage ≧ 70%
...
DC パイプラインは JS だけでなく Python・C#・Rust にも応用済み (`RQ3/Code/`)。

| 言語 | DC1 対象ファイル | 実装場所 |
|:---|:---|:---|
| JavaScript | `package.json` | `DC/data-curation-all/` |
| Python | `pyproject.toml` | `RQ3/Code/python/` |
| C# | `.csproj` | `RQ3/Code/csharp/` |
| Rust | `Cargo.toml` | `RQ3/Code/rust/` |

DC2・ci-check は `RQ3/Code/common/` に共通実装。スキップ判定条件は全言語で同一。  
JS版は5モデル (depcheck + knip + 3 LLM)、他言語版は **LLM 3モデルのみ** (depcheck・knip がJS専用のため)。
